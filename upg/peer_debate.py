...
Enables direct agent-to-agent communication without routing every message
through the PI.  Supports:

    PeerDebate       – structured two-agent debate on a scientific question
    PanelDiscussion  – N agents debate round-robin until consensus/timeout
    SocraticDialog   – one agent probes another with targeted questions

All debates produce a DebateRecord that can be passed straight into the
HybridOrchestrator's meeting context.
"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "turns": [
                {
                    "agent": t.agent_role,
                    "content": t.content[:300],   # truncate for context injection
                    "agrees": t.agrees,
                }
                for t in self.turns
            ],
...
class PeerDebate:
    ...
    def _moderate(self, record: DebateRecord, topic: str) -> Dict[str, Any]:
        ...
        prompt = (
            ...
            f"TRANSCRIPT:\n{turns_text}\n\n"
            "Return JSON: {consensus: str|null, key_disagreements: [], "
            "recommendation: str, areas_of_agreement: []}"
        )
        try:
...
# ---------------------------------------------------------------------------
# Panel discussion (N agents)
# ---------------------------------------------------------------------------

class PanelDiscussion:
    """
    Round-robin panel where N agents each get to speak, then respond to others.

    Stops early if consensus is reached or after `max_rounds`.
    """
    ...
    def run(
        ...
        record = DebateRecord(topic=topic)
        panel_memory: List[str] = []   # running transcript summary

        for round_num in range(max_rounds):
            for agent in self.agents:
                prompt_ctx = {
                    **context,
                    "panel_so_far": panel_memory[-6:],   # last 6 turns
                    "round": round_num + 1,
                }
    ...
    def _check_consensus(
        self, recent_turns: List[Turn], topic: str
    ) -> Dict[str, Any]:
...
class SocraticDialog:
    """
    One questioner agent probes a responder agent with targeted questions
    to surface assumptions and strengthen reasoning.
    """
    ...
    def run(
        ...
        # Responder states initial position
        ...
        for i in range(self.n_questions):
            ...
            question_resp = self.questioner.analyze(
                ...
                (
                    f"You are probing the {self.responder.role}'s reasoning. "
                    f"Ask a sharp Socratic question (round {i+1}/{self.n_questions})."
                ),
            )
            question_text = question_resp.get(
                "assessment", json.dumps(question_resp)
            )
            record.turns.append(Turn(
