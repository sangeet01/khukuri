...
Both expose a unified run_parallel(tasks) -> List[Result] interface so the
orchestrator doesn't care which backend is used.

Also ships a streaming progress reporter so long multi-step runs feel alive.
"""
...
class ProgressReporter:
    ...
    def update(self, result: TaskResult):
        ...
        self._on_update(
            f"[{pct:3d}%] {status} {result.task_id} "
...
# ---------------------------------------------------------------------------
# Thread-based parallel executor (sync-safe)
# ---------------------------------------------------------------------------
...
class ParallelExecutor:
    ...
    Safe to use in regular (non-async) code.  Each LLM call blocks a thread
    but that's fine since they're I/O bound.
    ...
    def run_parallel(
        ...
        while pending:
            ...
            # Also include tasks whose deps failed (they'll be skipped)
            blocked_by_failure = [
                t for t in pending.values()
                if any(
            ...
            if not ready:
                if pending:
                    logger.error("Dependency deadlock detected, aborting remaining tasks")
            ...
            with ThreadPoolExecutor(max_workers=min(self.max_workers, len(ready))) as pool:
                future_to_task = {
                    pool.submit(self._run_task, t): t
                    for t in ready
                }
                for future in as_completed(future_to_task,
                                           timeout=timeout_per_task * len(ready)):
...
# ---------------------------------------------------------------------------
# Asyncio executor (for async-native codebases)
# ---------------------------------------------------------------------------
...
class AsyncExecutor:
    ...
    Use when callers are already in an async context.
    Wraps synchronous callables in run_in_executor automatically.
    """
    ...
    def run_parallel(self, tasks: List[Task], **kwargs) -> List[TaskResult]:
        ...
        try:
            loop = asyncio.get_event_loop()
            if loop.is_running():
                # We're inside an existing loop (e.g., Jupyter)
                import nest_asyncio
                nest_asyncio.apply()
        except RuntimeError:
...
def make_executor(
    ...
    mode: "parallel" (thread-based, default) | "async" (asyncio-based)
    """
    if mode == "async":
        return AsyncExecutor(max_concurrency=max_workers, on_progress=on_progress)  # type: ignore
