...
Storage layout (all under `memory_dir`, default ~/.khukuri/memory/):
    failures.json          – FailureAnalyzer database + patterns
    meetings.json          – condensed meeting log
    world_state.json       – WorldStateTracker snapshot
    constraints.json       – design constraints learned over time
    session_log.json       – lightweight audit trail of every session
"""
...
class PersistentMemory:
    ...
    Usage:
        ...
        data = mem.load("failures")          # returns {} if not found
        mem.append("meeting_log", meeting)   # append one item to a list key
    """
    ...
    def append(self, key: str, item: Any) -> bool:
        ...
        # Cap lists at 1000 items to prevent unbounded growth
    ...
    # ------------------------------------------------------------------
    # Convenience helpers used by agent subsystems
    # ------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Mixin – add to any agent class that needs persistence
# ---------------------------------------------------------------------------
...
class MemoryMixin:
    ...
    Classes must define `_memory_key` (str) and `_state_to_dict()` /
    `_load_from_dict(data)` methods.
    """
...
def _serialise(obj: Any) -> Any:
    ...
    if hasattr(obj, "__dict__"):             # arbitrary objects
        return obj.__dict__
    return str(obj)
